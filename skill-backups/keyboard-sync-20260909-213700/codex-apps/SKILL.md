...
description: 为 Windows computer-use 提供 QQ、VS Code、Firefox 和 Altium Designer 的应用快捷键与键盘工作流。在按钮难辨认、需要命令面板、搜索、菜单或编辑器导航时按需使用；区分组合键、顺序键、当前焦点与版本差异。
---
...
- 已核对的 WorkBuddy `local-computer-tools`（2026-09-09）只开放 `ENTER, TAB, ESC, BACKSPACE, DELETE, UP, DOWN, LEFT, RIGHT, HOME, END, PAGEUP, PAGEDOWN, CTRL+A, CTRL+F, CTRL+L, CTRL+S, CTRL+Z, CTRL+Y, SHIFT+TAB`。运行时重新发现 schema；未扩展前，本技能里的其他键位只能通过已有菜单/结构化能力替代或供用户手动操作。
- 缺少某个快捷键时，不用文字输入模拟按键，不向终端注入自动化脚本，不暗改工具白名单。主技能要求的截图、焦点和一次输入令牌仍然有效。
