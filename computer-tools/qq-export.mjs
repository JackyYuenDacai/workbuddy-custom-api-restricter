...
export function registerQQExportTools(server,run=runQQExport) {
  ...
  server.registerTool('qq_export_messages',{description:'Plan or submit a local QCE message export for one exact group/friend ID and explicit date interval. dry_run defaults true and performs no network request. Set false for the user-authorized export after resolving identity. YYYY-MM-DD dates use UTC+8; end date includes that entire day. ISO timestamps require offset and end is EXCLUSIVE. Optional sender_ids selects senders within the conversation. Media downloads default off; set include_media=true to download resources, or embed_media=true to request images/resources embedded as data URIs (this also enables media downloads and can produce very large files). Returns job_id for polling; accepted is not completed. Requires an already running authenticated QCE backend; never retries submissions or sends QQ messages.',inputSchema:{kind,chat_id:id,start:z.string().min(10).max(40),end:z.string().min(10).max(40),format:z.enum(['JSON','TXT','HTML','EXCEL']).default('JSON'),sender_ids:z.array(id).max(50).default([]),include_media:z.boolean().default(false),embed_media:z.boolean().default(false),dry_run:z.boolean().default(true)},annotations:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:true}},wrap('export'));
